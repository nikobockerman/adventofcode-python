...
def _multiple_problems(answers: Iterable[answers.Answer]) -> int:
    all_passed = None
    slowest = None
    start = time.perf_counter()
    with joblib.Parallel(n_jobs=-1, return_as="generator") as parallel:
        inputs = [_get_problem_input(answer) for answer in answers]
...
def _get_problem_input(id_: answers.ProblemId) -> _ProblemInput:
    try:
        mod_name = f"adventofcode.y{id_.year}.d{id_.day}"
